...
PlaySocket.prototype = {
    ready: function() {
    	...
    	this.socket.on('connetion_break', function(control) {
    ...
    playGame: function() {
        if (this.stage == 0) {
            ...
            var ind_syst = this.selected_syst.id.split('_')[1];
            var card_oppo = this.game.cards[ind_oppo];
            var card_syst = this.game.cards[ind_syst];
            var guess = -1;
            while (true) {
    ...
    showFuncChoice: function() {
        var cards = this.game.cards;
        var html = '<button class="card btn btn-hg btn-primary btn-block back" onclick="play_socket.selectFunc(-1)"></button>';
        var mine = this.game.player[this.username];
        var st = this.stage == 0 ? (this.func_card == 3 ? 2 : 1) : 0;
        for (var i = st; i < mine.length; i++) {
            html += this.getHtml(cards[mine[i]], 'mine');
            html += '<button class="card btn btn-hg btn-primary btn-block back" onclick="play_socket.selectFunc(' + i + ')"></button>';
        }
        $('div#card_mine').html(html);   
    },
    selectFunc: function(ind) {
        var arr = new Array();
        var mine = this.game.player[this.username];
    ...
    insertCard: function(card) {
        if (card.number == 12) {
            ...
            $('div#play_info').html('Please select where to place your ' + (card.color ? 'white' : 'black') + ' functional card.');
            this.showFuncChoice();
            this.func_card = 0;
        } else {
            var mine = this.game.player[this.username];
